...
{
    // Vanilla-Plus (W3-06): the ladder is capped at 5 gold -- 1g first, 5g every
    // time after -- below the stock core's native 10g third step, so the
    // month-since-last-reset decay below (which only ever fires once
    // m_resetTalentsCost reaches 10g, and floors at 10g) can no longer be
    // reached; removed rather than left as dead code that implies a decay this
    // ladder no longer has. m_resetTalentsCost never exceeds 5g under this
    // ladder, so it also never falls below the 1g floor.
